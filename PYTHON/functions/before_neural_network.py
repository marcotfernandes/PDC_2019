...
def checkgrad(Srx, Stx, nSamples, p_train, p_val):
    ...
    trash, grad = DNN.nnCostFunction(theta, input_layer_size, hidden_layer_size, output_layer_size,
                       X_train, Y_train, lambda_r)

    function_handler = lambda x : DNN.nnCostFunction(x, input_layer_size, hidden_layer_size,
                                                     output_layer_size, X_train, Y_train, lambda_r)

    numGrad = computeNumericalGradient(function_handler, theta)

    diff = np.linalg.norm(numGrad[0:10] - grad[0:10])/np.linalg.norm(numGrad[0:10] + grad[0:10])
