...
def unravel(Stx, Srx, nSamples, nodes, lambda_r, num_iter_train, p_train, p_val):
    ...
    s = []
    s1 = list([Stx, Srx, nSamples[0], 4, lambda_r[0], num_iter_train, p_train, p_val])
    s2 = list([Stx, Srx, nSamples[0], 4, lambda_r[0], num_iter_train, p_train, p_val])
    ...
    for i in range(0, nSamples_size):
        for j in range(0, lambda_size):
            for k in range(0, nodes_size):
                ...
                nodes_aux = nodes[k]
                s1 = list([Stx, Srx, nSamples_aux, nodes_aux, lambda_aux, num_iter_train, p_train, p_val])
                s.append(s1)
    return s
